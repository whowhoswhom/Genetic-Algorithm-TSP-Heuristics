...
def parse_tsp_file(filename):
    ...
    formatted as "index x y".
    The function ignores any lines outside of this section.
    """
    cities = []
...
def aggregate_expert_routes(experts, num_cities):
    ...
    while remaining_cities:
        ...
        if max(consensus_route) >= len(cities) or min(consensus_route) < 0:
            print("Error: Consensus route indices exceed city dataset.")
            return []
...
def smart_swap_mutation(individual, mutation_rate):
    ...
    if random.random() < mutation_rate:
        ...
        if len(set(individual)) != len(individual):
            print("Error: Mutation resulted in duplicate cities.")
    return individual
...
def heuristic_edge_crossover(parent1, parent2):
    ...
    if len(set(child)) != len(child):
        print("Error: Child route has duplicates after crossover.")
    return child
...
def calculate_fitness(route, distances):
    if len(set(route)) != len(route):  # Ensure no duplicate cities
        print("Invalid route detected in fitness calculation.")
        return float('-inf')  # Assign a very low fitness
...
def genetic_algorithm_cpp(cities, population_size=50, generations=100,
    ...
    for gen in tqdm(range(generations)):
        ...
        # Create a new population with consensus route as elite every 5 generations
...
def plot_route(cities, route):
    ...
    """
    if len(route) != len(cities) or max(route) >= len(cities) or min(route) < 0:
        print("Error: Invalid route passed to plot_route.")
        return
    ordered_cities = [cities[i] for i in route]
    ordered_cities.append(ordered_cities[0])  # To close the loop
...
if __name__ == "__main__":
    ...
    # Prompt user for the TSP file path
    tsp_file = input("Enter the path to the .tsp file: ") or 'C:/Users/josef/PycharmProjects/FP-CSE545/Random100.tsp'
    if not os.path.exists(tsp_file):
        print(f"Error: File {tsp_file} not found.")
        exit(1)

    # Parse the TSP file to extract cities

    try:
        population_size = int(input("Enter population size (default is 50) (from 1-100): ") or 50)
        if not 1 <= population_size <= 100:
            raise ValueError("Population size must be between 1 and 100.")
    except ValueError as ve:
        print(f"Invalid input: {ve}. Using default value of 50.")
        population_size = 50

    # Parse the TSP file to extract cities
    cities = parse_tsp_file(tsp_file)
    cities = cities[:population_size]  # Limit to the first 'population_size' cities

    try:
        generations = int(input("Enter number of generations (default is 100) (from 1-500): ") or 100)
        if not 1 <= generations <= 500:
            raise ValueError("Generations must be between 1 and 500.")
    except ValueError as ve:
        print(f"Invalid input: {ve}. Using default value of 100.")
        generations = 100

    try:
        mutation_rate = float(input("Enter mutation rate (default is 0.1%) (from 0.1% to 10%): ") or 0.1)
        if not 0.1 <= mutation_rate <= 10.0:
            raise ValueError("Mutation rate must be between 0.1% and 10%.")
    except ValueError as ve:
        print(f"Invalid input: {ve}. Using default value of 0.1%.")
        mutation_rate = 0.1

    # Run the genetic algorithm
    best_route, best_coverage, progress = genetic_algorithm_cpp(
        cities, population_size=population_size, generations=generations, mutation_rate=mutation_rate
    )

    # Ensure the algorithm returned valid results
    if not best_route or not progress:
        print("Error: Genetic algorithm did not return a valid solution.")
        exit(1)
    ...
    plt.grid(True)
    plt.show()
    if max(best_route) >= len(cities) or min(best_route) < 0:
        print("Error: Best route indices exceed city dataset.")
        exit(1)
    ...
    print("Route Length:", len(best_route))
    print("Unique Cities in Route:", len(set(best_route)))
